...
export default function App({ amplifyReady, auth }: AppProps) {
  ...
  const backendHealth: BackendHealthType = amplifyReady
    ? { status: isOnline ? 'ok' : 'offline', detail: isOnline ? 'Conexión lista' : 'Sin conexión' }
    : { status: 'checking', detail: 'Backend no inicializado' }

  return (
    <div className="min-h-dvh bg-zinc-50 text-zinc-950 dark:bg-zinc-950 dark:text-zinc-50">
      <Toast toast={toast} onClear={() => setToast(null)} />
      <header className="border-b border-zinc-200/70 bg-white/70 px-4 py-3 shadow-sm sticky top-0 z-10 dark:border-zinc-800/60 dark:bg-zinc-950/40 backdrop-blur-md">
      ...
      <div className="flex">
        ...
        <main className="mx-auto max-w-6xl px-4 py-6 w-full">
          {sidebarActive === 'dashboard' ? <DashboardModule amplifyReady={amplifyReady} isOnline={isOnline} /> : null}
