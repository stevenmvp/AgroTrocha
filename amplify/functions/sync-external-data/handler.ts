...
// Minimal safe handler: performs a lightweight auth check and returns
// a placeholder message. This avoids merge-conflict and lint issues
// while keeping the function deployable. Full sync logic can be
...
export const handler: Schema['syncExternalApiNow']['functionHandler'] = async (event) => {
  ...
  const groups = Array.isArray(groupsRaw) ? groupsRaw.map(String) : typeof groupsRaw === 'string' ? groupsRaw.split(',').map((s) => s.trim()) : []
